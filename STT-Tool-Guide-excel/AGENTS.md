...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-100 to-slate-200">
  <div class="text-center">
    <h1 class="text-2xl font-semibold text-slate-800">Welcome to   </h1>
  </div>
</div>
```
...
- Commands are loaded from `public/data/commands.json` via the API service.
- Prerequisites and property screens load JSON directly from `public/data/*` using `PrerequisiteService`.
- If you edit `commands.xlsx`, regenerate JSON with `npm run xlsx-to-json` before testing.

### Routes

- Command page: `/cmd/:id`
- Prerequisites pages: `/prerequisites/*`
  - excel, html, locators, locator-config, testing-properties, user-profile

### TOC Scroll Behavior

The command page uses a right-side TOC that scrolls the main content area, not the window.
The layout wraps routed content inside a scrollable `main` element (`overflow-y-auto`).

Key details:

- TOC click handler scrolls the `main` container directly to avoid jumping or blank space at the bottom.
- Section headings use `scroll-mt-24` to keep headings visible when scrolled into view.
- The command page adds bottom padding so the last section can scroll to the top cleanly.

If you change layout or scroll container, review:

- [src/app/components/table-of-contents/table-of-contents.ts](src/app/components/table-of-contents/table-of-contents.ts)
- [src/app/components/command-section/command-section.html](src/app/components/command-section/command-section.html)
- [src/app/components/command-page/command-page.scss](src/app/components/command-page/command-page.scss)
